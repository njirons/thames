...
thames <- function(lps = NULL,params,n_samples = NULL,d = NULL, radius = NULL,
  ...
  # correct coefficient of variation for autocorrelation in MCMC samples
  cv <- sd(exp(-lps2+max(lps2))*inA)/
    ((1-phi)*sqrt(n2)*mean(exp(-lps2+max(lps2))*inA))
  
  # lower bound of confidence interval
  log_zhat_inv_L <- log_zhat_inv + log(trunc_quantile(p,cv))
  
  # upper bound of confidence interval
  log_zhat_inv_U <- log_zhat_inv + log(trunc_quantile(q,cv))
