...
res_old = cbind(res_unif_bound, res_unif_nobound, res_random_bound, res_random_nobound)
#write.matrix(res_old,'data/multinomial.csv')
    ...
    this <- foreach(i=1:y_sims, .combine=cbind,
      ...
      if(c==1){
        ...
        log_bridge = bridge_sampler(samples=transformed_samples,
                                    ...
                                    +sum(log(abs(eigen(cbind(0,diag(length(param.row)))%*%soft_max_der(smax(c(-sum(param.row),param.row)))%*%
